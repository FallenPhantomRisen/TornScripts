...
(function() {
    ...
    let apiKey = GM_getValue("API_KEY", ""); // Try to retrieve the API key from storage
    ...
    // Create a Map to keep track of players and their statuses
    const idList = new Map();

    // Initialize the box for UI display
    function initBox() {
    ...
    // Your existing logic to fetch player data
const fetchPlayerData = async (playerId) => {
    ...
    return {
        ...
        age: data.age  // Add this line for age
    };
};
...
async function refreshBox() {
    // If the box is empty, return early
    if (!idList.size) {
        return;
    }
    ...
    for (let [username, data] of idList.entries()) {
        ...
        // If a player has left the table, add the "Mug" button
        if (data.leftTable) {
            const mugLink = document.createElement('a');
            mugLink.textContent = 'Mug';
            mugLink.href = `https://www.torn.com/loader.php?sid=attack&user2ID=${playerId}`;
...
async function forceRecheckNA() {
    ...
    // After updating, call refreshBox again to reflect the changes
    refreshBox();
}
    ...
    // Check players with class 'opponent___ZyaTg default___BKxzS'
    checkPlayers('.opponent___ZyaTg.default___BKxzS[id^="player-"]', existingPlayerIds);
    // Check players with class 'opponent___ZyaTg folded___jXomW'
    checkPlayers('.opponent___ZyaTg.folded___jXomW[id^="player-"]', existingPlayerIds);

    // Set isPresent flag back to false for the next run
    ...
    setInterval(forceRecheckNA, 5000); // Automatically run every 5 seconds
